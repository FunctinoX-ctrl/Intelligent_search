...
// 消息相关 IPC
ipcMain.handle('messages:get-list', async (event, page, pageSize, type) => {
  try {
    return await apiRequest(`/api/messages/list?page=${page || 1}&pageSize=${pageSize || 20}&type=${type || 'all'}`)
  } catch (error) {
    return { success: false, message: '连接服务器失败: ' + error.message }
  }
})
...
ipcMain.handle('messages:get-unread-count', async () => {
  try {
    return await apiRequest('/api/messages/unread-count')
  } catch (error) {
    return { success: false, message: '连接服务器失败: ' + error.message }
  }
})

ipcMain.handle('messages:mark-read', async (event, id) => {
  try {
    return await apiRequest(`/api/messages/read/${id}`, { method: 'POST' })
  } catch (error) {
    return { success: false, message: '连接服务器失败: ' + error.message }
  }
})
...
ipcMain.handle('messages:mark-all-read', async () => {
  try {
    return await apiRequest('/api/messages/read-all', { method: 'POST' })
  } catch (error) {
    return { success: false, message: '连接服务器失败: ' + error.message }
  }
})
